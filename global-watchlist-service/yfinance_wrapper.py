...
def global_indices():
    """
    Returns live price data for major global market indices.
    ...
    Called by global-watchlist-service YahooFinanceClient on port 5002.
    """
    result = []
    ...
    try:
        ...
        for sym in symbols:
            try:
                ...
                app.logger.warning("global-indices: skipping %s — %s", sym, e)
    except Exception as e:
        app.logger.error("global-indices batch fetch failed: %s", e)
...
if __name__ == '__main__':
    ...
    print(f"Starting yfinance wrapper on port {args.port}...")
